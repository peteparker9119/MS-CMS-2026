...
const IcBell    = ({ color }) => <SvgIcon color={color}><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 01-3.46 0"/></SvgIcon>;
...
function FieldRow({ icon, children, top = false }) {
  return (
    <div style={{ display: 'flex', gap: 16, alignItems: top ? 'flex-start' : 'flex-start', padding: '5px 0' }}>
      <div style={{ width: 20, flexShrink: 0, paddingTop: top ? 10 : 9, display: 'flex', justifyContent: 'center' }}>
        {icon}
      </div>
...
function TimePicker({ value, onChange, label }) {
  const opts = useMemo(() => {
    ...
    for (let h = 0; h < 24; h++) {
      for (let m = 0; m < 60; m += 15) {
        ...
        const ap  = h >= 12 ? 'PM' : 'AM';
        const hr  = h > 12 ? h - 12 : h === 0 ? 12 : h;
        list.push({ v: t24, l: `${hr}:${String(m).padStart(2, '0')} ${ap}` });
      }
    }
    return list;
  }, []);
  ...
  return (
    <div style={{ position: 'relative', flex: 1 }}>
      {label && <div style={{ fontFamily: RI, fontSize: 10, color: '#80868b', marginBottom: 3, textTransform: 'uppercase', letterSpacing: '.5px' }}>{label}</div>}
      <select value={value} onChange={e => onChange(e.target.value)}
        style={{ width: '100%', border: '1px solid #dadce0', borderRadius: 4, padding: '7px 10px', fontSize: 13, fontFamily: RI, color: '#3c4043', background: '#fff', outline: 'none', cursor: 'pointer', appearance: 'none', WebkitAppearance: 'none' }}
...
export default function EventModal({ initial, pairs, meetings, onSave, onClose, saving = false }) {
  ...
  // Auto-notify pair units on first pair selection
  useEffect(() => {
  ...
  const handleSave = () => {
    ...
    // Only include status in update (backend handles it on create = 'scheduled')
  ...
  return (
    <>
      {/* Scrim */}
      <div onClick={onClose}
        style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 1049, backdropFilter: 'blur(1px)' }} />
      ...
      <div style={{
        ...
        background: '#fff',
        borderRadius: 12,
        boxShadow: '0 24px 48px rgba(0,0,0,.2), 0 8px 16px rgba(0,0,0,.1)',
        ...
        {/* ── Top color bar ── */}
        <div style={{ height: 6, background: editing ? statusColor : '#1a73e8', flexShrink: 0, borderRadius: '12px 12px 0 0' }} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 20px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          <FieldRow icon={<IcClock />}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {durStr && (
                  <span style={{ fontFamily: RI, fontSize: 12, color: '#80868b', flexShrink: 0, whiteSpace: 'nowrap' }}>
                    {durStr}
                  </span>
                )}
              </div>
          ...
          {/* Recurrence */}
          <FieldRow icon={<IcRepeat />}>
            <select value={recurrence} onChange={e => setRecurrence(e.target.value)}
              style={{ width: '100%', border: '1px solid #dadce0', borderRadius: 4, padding: '7px 10px', fontSize: 13, fontFamily: RI, color: '#3c4043', background: '#fff', outline: 'none', cursor: 'pointer' }}
              onFocus={e => { e.target.style.borderColor = '#1a73e8'; e.target.style.boxShadow = '0 0 0 2px rgba(26,115,232,.2)'; }}
              onBlur={e  => { e.target.style.borderColor = '#dadce0'; e.target.style.boxShadow = 'none'; }}>
          ...
          {/* Convergence Unit Pair */}
          <FieldRow icon={<IcUsers />}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <select value={pairId}
                onChange={e => { setPairId(e.target.value); setNotify([]); }}
                style={{ width: '100%', border: '1px solid #dadce0', borderRadius: 4, padding: '7px 10px', fontSize: 13, fontFamily: RI, color: pairId ? '#3c4043' : '#80868b', background: '#fff', outline: 'none', cursor: 'pointer' }}
                onFocus={e => { e.target.style.borderColor = '#1a73e8'; e.target.style.boxShadow = '0 0 0 2px rgba(26,115,232,.2)'; }}
                onBlur={e  => { e.target.style.borderColor = '#dadce0'; e.target.style.boxShadow = 'none'; }}>
                <option value="">— Select convergence unit pair —</option>
                {pairs.map(p => (
                  <option key={p.id} value={p.id}>{p.unit_a.name} × {p.unit_b.name}</option>
                ))}
              </select>

              {/* Pair unit notify chips */}
              {pairUnits.length > 0 && (
                <div>
                  <div style={{ fontFamily: RI, fontSize: 11, color: '#80868b', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.5px' }}>Notify teams</div>
                      ...
                      return (
                        <label key={u.id}
                          ...
                          <input type="checkbox" checked={checked}
                            onChange={() => setNotify(n => checked ? n.filter(x => x !== u.id) : [...n, u.id])}
          ...
          {hasConflict && (
            <div style={{ margin: '6px 0 2px', padding: '10px 14px', background: '#fef7e0', border: '1px solid #f9ab00', borderRadius: 6, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div style={{ fontFamily: RI, fontSize: 12, color: '#b05e00', lineHeight: 1.5 }}>
                <strong>Scheduling conflict</strong> — another meeting overlaps this time slot. You can still save, but it will show as a conflict.
              </div>
            </div>
          )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderTop: '1px solid #e8eaed', flexShrink: 0, background: '#fff' }}>
          <span style={{ fontFamily: RI, fontSize: 11, color: hasConflict ? '#b05e00' : '#80868b' }}>
            {hasConflict ? '⚠️ Conflict detected — still saveable' : 'Ctrl+Enter to save'}
          </span>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={onClose}
              style={{ border: '1px solid #dadce0', borderRadius: 4, padding: '8px 22px', fontSize: 14, fontFamily: GS, cursor: 'pointer', background: '#fff', color: '#1a73e8', fontWeight: 500 }}
              onMouseEnter={e => e.currentTarget.style.background = '#f8f9fa'}
              onMouseLeave={e => e.currentTarget.style.background = '#fff'}>
              Cancel
            </button>
            <button type="button" onClick={handleSave} disabled={!canSave}
              style={{ border: 'none', borderRadius: 4, padding: '8px 24px', fontSize: 14, fontFamily: GS, fontWeight: 500, cursor: canSave ? 'pointer' : 'not-allowed', background: canSave ? '#1a73e8' : '#c2d6f5', color: '#fff', transition: 'background .13s', minWidth: 80 }}
